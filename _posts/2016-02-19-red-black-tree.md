...
### 删除
和插入相反，若目标结点所包含的元素多于1个，说明元素有多余，直接删除目标元素即可；若结点只包含1个
元素，则删除元素后结点为空，需要从相邻结点补充元素，如果相邻结点也没有多余的元素，则从父结点补充
...
#### 注意
为了简化实现，上述代码中的`search`和`remove`方法都直接返回的结点指针，因此需要在类的外部单独
